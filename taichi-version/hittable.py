...
def hit_triangle(v1, v2, v3, nn, ray_origin, ray_direction, t_min, t_max):
    ...
    if theta < 0:
        ...
        w1 = (r_interact-v1).cross(r_interact-v2).norm() / \
            (v3-v1).cross(v3-v2).norm()
        w2 = (r_interact-v1).cross(r_interact-v3).norm() / \
            (v2-v1).cross(v2-v3).norm()
        w3 = (r_interact-v3).cross(r_interact-v2).norm() / \
            (v1-v3).cross(v1-v2).norm()
        w = Vector(w1, w2, w3)
...
class Triangle:
    def __init__(self, v1, v2, v3, u1, u2, u3, material):
        ...
        try:
            self.box_min = [
                min(v1(0), v2(0), v3(0)), min(
                    v1(1), v2(1), v3(1)), min(v1(2), v2(2), v3(2))
            ]
            self.box_max = [
                max(v1(0), v2(0), v3(0)), max(
                    v1(1), v2(1), v3(1)), max(v1(2), v2(2), v3(2))
            ]
        except TypeError:
            self.box_min = [
                min(v1[0], v2[0], v3[0]),
...
class World:
    ...
    def commit(self):
        ...
        ti.root.dense(ti.i, self.n).place(self.radius, self.center,
                                          self.v1, self.v2, self.v3, self.u1, self.u2, self.u3, self.nor)
        ...
        while curr != -1:
            ...
            if obj_id != -1:
                ...
                if hit:
                    ...
                    uv = self.u1[obj_id]*w[0] + self.u2[obj_id] * \
                        w[1] + self.u3[obj_id]*w[2]
